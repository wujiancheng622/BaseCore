...
public:
	...
	int SetPackageHeaderTrailer(const char *pHeader,int headerLen,const char *pTrailer,int trailerLen)
	{
		...
		if (!Is_Started())
		{
			std::cout<<"SetPackageHeaderTrailer enter"<<std::endl;
			string headStr;
			string tailerStr;
			...
			std::cout<<"SetPackageHeaderTrailer leave success"<<std::endl;
			return 0;
		}else{
			return -1;
		}
		...
		if(sock_tmp)
		{
			...
			if (m_bGroupPackage)
			{
				...
				if(bAsync)
				{
					...
					//cout<<"ret"<<ret<< "m_packetHeaderLen "<<m_packetHeaderLen<<" m_packetTailerLen "<<m_packetTailerLen<<" totalLen"<<totalLen<<endl;
					if (ret>0)
					{
